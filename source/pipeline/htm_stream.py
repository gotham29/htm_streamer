...
from source.config.config import load_config, save_config, build_enc_params, extend_targets_samples
...
def stream_to_htm(config_path, data_path):
    ...
    if cfg['iter_current'] < cfg['iter_samplesize']:
        ...
        if cfg['iter_current'] == 0:
            cfg['targets_samples'] = {f:[] for f in cfg['targets']}
        else:
            cfg['targets_samples'] = extend_targets_samples(cfg['targets_samples'], data)
    ...
    #     d. Store —> Models
        ...
        mode = 'init_models'
        cfg['targets_samples'] = extend_targets_samples(cfg['targets_samples'], data)
        targets_enc_params = build_enc_params(cfg['targets_samples'],
                                               cfg['models_enc_n'],
    ...
    #     e. Store —> Models (to: cfg['dir_models'])
        ...
        mode = 'run_models'
        targets_models = load_models(cfg['dir_models'], cfg['targets'])
