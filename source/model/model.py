...
class HTMModel:
    ...
    def get_preds(self, iter, f_data):
        ...
        self.predictor.learn(iter, self.tm.getActiveCells(),
                             int(f_data / self.predictor_resolution))
        return steps_predictions

    def run(self, features_data, iter, learn, predictor_config):
        # ENCODERS
        ...
        if predictor_config['enable'] and len(self.features_model) == 1:
            ...
            steps_predictions = self.get_preds(iter=iter,
...
def run_models(iter, data, learn, features_models, timestamp_config, predictor_config):
