...
class HTMmodel:
    def __init__(self,
        ...
        self.predictive_cells = SDR(dimensions=(self.sp.getNumColumns(), self.tm.getCellsPerColumn()))
        ...
        if self.use_predictor:
            ...
            self.predictor = Predictor(steps=self.predictor_steps_ahead,
                                       alpha=predictor_config['sdrc_alpha'])
    ...
    def init_sp(self) -> Union[None, SpatialPooler]:
        ...
        if self.use_spatial_pooler:
            return SpatialPooler(
                ...
                stimulusThreshold=self.models_params["sp"]["stimulusThreshold"],
    ...
    def run(self,
        ...
        self.tm.activateCells(activeColumns=active_columns, learn=learn)
        winners = self.tm.getWinnerCells()

        # check anomaly
        anomaly_score = 1. - self.predictive_cells.getOverlap(winners) / winners.getSum()
        ...
        if self.single_feature and self.use_predictor and self.iteration_ > 1000:
        ...
        return anomaly_score, anomaly_likelihood, None, steps_predictions
